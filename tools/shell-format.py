...
def check_script(script: str) -> str:
    ...
    """
    # Check using shfmt
    shfmt = subprocess.run(["shfmt", "-d", src/rootfstool/rootfstool src/container], capture_output=True)
    if shfmt.returncode != 0:
        return "Step 1 Failed."

    # Check using shellcheck
    shellcheck = subprocess.run(["shellcheck", src/rootfstool/rootfstool src/container], capture_output=True)
